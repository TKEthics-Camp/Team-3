...
The optional service in `server/index.js` provides JSON APIs for registration, login, revisioned save synchronization, mutual friendships, time-windowed leaderboards, matchmaking, and match event relay. Browser clients use ordinary HTTP requests and Server-Sent Events. The demo configuration connects on port `8787` and normalizes direct-file, `localhost`, and IPv6-loopback pages to the explicit IPv4 loopback host `127.0.0.1` for consistent Windows, Linux, and macOS behavior. Other page hostnames remain available for non-loopback deployments. Host, port, and protocol are independently configurable in `src/config/remote.js`.
