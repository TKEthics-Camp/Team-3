function initStickSetup(){
  canvas.hidden=true;dom.hidden=false;controls([]);$('#helpText').textContent='';setStatus(L('选择对战模式','Choose battle mode'));
  const settingsStore='gongxing_stick_settings_v1';
  let saved={mode:'ai',level:'normal'};
  try{const value=JSON.parse(localStorage.getItem(settingsStore)||'null');if(value?.mode==='pvp'||value?.mode==='ai')saved.mode=value.mode;if(['easy','normal','hard'].includes(value?.level))saved.level=value.level}catch{}
  dom.innerHTML=`<div class="mode-panel"><h3>${L('⚔️ 火柴人格斗 · 雷霆对决','⚔️ Stick Fighter · Thunder Duel')}</h3><p>${L('先选择真人双人或 AI 对手；选择 AI 时还可决定挑战强度，确认后再进入擂台。','Choose local PVP or an AI opponent. AI mode also lets you select the challenge level before entering the arena.')}</p><div class="setup-grid"><label class="setup-field">${L('对战模式','Battle mode')}<select id="stickSetupMode"><option value="ai">${L('对战 AI','Vs AI')}</option><option value="pvp">${L('本地真人双人','Local PVP')}</option></select></label><label class="setup-field" id="stickSetupDifficultyField">${L('AI 难度','AI difficulty')}<select id="stickSetupDifficulty"><option value="easy">${L('简单','Easy')}</option><option value="normal">${L('普通','Normal')}</option><option value="hard">${L('困难','Hard')}</option></select></label></div><button class="setup-start" id="stickSetupStart">${L('开始对战','Start Battle')}</button></div>`;
  const mode=$('#stickSetupMode'),difficulty=$('#stickSetupDifficulty'),field=$('#stickSetupDifficultyField');
  mode.value=saved.mode;difficulty.value=saved.level;field.hidden=mode.value==='pvp';
  mode.onchange=()=>field.hidden=mode.value==='pvp';
  $('#stickSetupStart').onclick=()=>{localStorage.setItem(settingsStore,JSON.stringify({mode:mode.value,level:difficulty.value}));launchStickMatch()};
...
function initStickPvp(){
  ...
  const loadSettings=()=>{try{const saved=JSON.parse(localStorage.getItem(settingsStore)||'null');return{mode:saved?.mode==='pvp'?'pvp':'ai',level:['easy','normal','hard'].includes(saved?.level)?saved.level:'normal'}}catch{return{mode:'ai',level:'normal'}}};
  ...
  g.key=(key,on)=>{
    ...
    for(const id of g.aiMode?[1]:[1,2])for(const action of actions)if(normalizeKey(keybinds[id][action])===normalized)return trigger(P(id),action,on)
  };
  ...
  g.destroy=()=>{try{audio?.close()}catch{}};setScore(0);setStatus(g.aiMode?L(`AI ${g.aiLevel==='easy'?'简单':g.aiLevel==='hard'?'困难':'普通'}模式`,`AI ${g.aiLevel.toUpperCase()} mode`):L('本地双人对决','Local PVP'));return g;
}
