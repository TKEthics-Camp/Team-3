...
const meta={
  ...
  stick:{zh:['火柴人格斗 · 光暗对决','开战前选择光或暗。暗系拥有十套独立连招与高速穿透攻击。','选择光或暗的独立技能组，挑战三档 AI、在线对手或本地双人。','格斗 · 光 / 暗'],en:['Stick Fighter · Light vs Dark','Choose Light or Dark before battle. Dark Magic has ten distinct combos and a high-speed phase attack.','Choose an independent Light or Dark move set and battle three AI levels, online opponents, or local PVP.','Fighting · Light / Dark']},
  ...
  tag:{zh:['3D 追逐战','使用 WASD 移动、空格跳跃，点击游戏画面锁定鼠标视角。','在立体地图中奔跑、跳跃并追逐机器人，体验多种地图与追逐模式。','3D 动作 · 单机机器人'],en:['3D Tag Arena','Use WASD to move, Space to jump, and click the game to capture the mouse.','Run, jump, and chase bots across 3D arenas with varied maps and tag modes.','3D Action · Solo Bots']}
};
...
function renderLanguage(){document.documentElement.lang=arcadeLang==='zh'?'zh-CN':'en';document.title=arcadeLang==='zh'?'无名游艺馆':'Nameless Arcade';document.querySelectorAll('[data-t]').forEach(e=>e.textContent=PAGE[arcadeLang][e.dataset.t]);for(const id of Object.keys(meta)){const m=meta[id][arcadeLang];$(`[data-game-title="${id}"]`).textContent=m[0];$(`[data-game-desc="${id}"]`).textContent=m[2];$(`[data-game-kind="${id}"]`).textContent=m[3]}$('#popularBadge').textContent=arcadeLang==='zh'?'🔥 人气超高！':'🔥 SUPER POPULAR!';$('#arcadeEn').textContent=arcadeLang==='zh'?'英文':'EN';$('#arcadeZh').textContent=arcadeLang==='zh'?'中文':'中文';$('#arcadeZh').className=arcadeLang==='zh'?'active':'';$('#arcadeEn').className=arcadeLang==='en'?'active':'';if(!active)$('#statusValue').textContent=PAGE[arcadeLang].ready;$('#closeGame').setAttribute('aria-label',L('关闭游戏','Close game'));syncScores()}
...
function startGame(){if(!validGameSession(gameSession))return expireGameAccess();mountGame(({gomoku:initGomoku,rhythm:initRhythm,reaction:initReaction,pacman:initPacman,stick:initStickSetup,chess:initChess,battleship:initBattleship,tag:initTag}[active]))}
