...
async function init(){loadAdminSettings();applyAdminMode();let bankFailed=false;try{const loaded=await Promise.all(QUESTION_BANK_SOURCES.map(loadQuestionBank));loaded.forEach(([source,bank])=>(source.mode==='hard'?hardBanks:banks)[source.subject]=bank)}catch(e){console.error(e);bankFailed=true;const detail=e instanceof Error?e.message:String(e);error(`题库加载或格式验证失败：${detail}。请确认仓库文件完整，并重新打开本页。`);document.querySelector('#start').disabled=true}loadUser();setLang(localStorage.getItem('gongxing_lang')==='en'?'en':'zh');const params=new URLSearchParams(location.search);if(!bankFailed&&params.has('gameExpired'))error('游戏时间已结束。继续答题赚取积分，即可再次兑换游戏时间。');else if(!bankFailed&&params.has('gameAccess'))error('请先使用积分兑换游戏时间。');if(params.has('gameExpired')||params.has('gameAccess'))history.replaceState({},'',location.pathname)}
